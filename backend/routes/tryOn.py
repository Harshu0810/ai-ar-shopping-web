...
# List of free public spaces hosting IDM-VTON
# If one is down, the code will automatically try the next.
VTON_PROVIDERS = [
    "yisol/IDM-VTON",           # Official (often busy)
    "Nymbo/Virtual-Try-On",     # Reliable Mirror
    "cuuupid/idm-vton",         # Backup Mirror 1
    "zeroshot/idm-vton-sku"     # Backup Mirror 2
]
...
async def generate_tryon(
    ...
    try:
        ...
        # 3. Upload User Image to Supabase
        ...
        # Fallback: Return original image to prevent frontend crash
        return {
            "success": False,
            "error": "AI Services busy, please try again later.",
            "original_image": user_photo_url if 'user_photo_url' in locals() else "",
            "generated_image": user_photo_url if 'user_photo_url' in locals() else "", # Fallback
